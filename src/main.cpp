...
//==============================================================================
inline int32_t match_length(int8_t const *beginInput,
    ...
    endInput += sizeof(int64_t);
    while ((inputB < endInput) && (*inputA == *inputB)) ++inputA, ++inputB;
    return (inputA - (beginInput + indexA));
}


//==============================================================================
void lcp(int8_t const *beginInput,
    int8_t const *endInput,
    int32_t *begin,
    int32_t size,
...
//==============================================================================
void lcp_multithreaded(int8_t const *beginInput,
    int8_t const *endInput,
    int32_t *begin,
    int32_t size,
    int32_t numThreads) {
...
//==============================================================================
void validate_lcp(int8_t const *beginInput,
    int8_t const *endInput,
    int32_t const *sa,
    int32_t size,
    int32_t const *lcp) {
...
//==========================================================================
void make_lcp_array(std::vector<int32_t> const &suffixArray, std::vector<int8_t> const &input, int32_t numThreads) {
    ...
    validate_lcp(
        input.data(), input.data() + input.size(), suffixArray.data() + 1, suffixArray.size() - 1, output.data());
}


//==============================================================================
std::vector<int8_t> load_file(std::string const &inputPath) {
...
bool write_file(std::string const &outputPath, input_iter begin, input_iter end) {
...
int compare(InputIter a, InputIter b, InputIter end) {
...
//==============================================================================
int32_t validate_suffix_array(std::vector<int8_t> const &input, std::vector<int32_t> const &suffixArray) {
    if (suffixArray[0] != (int32_t)input.size()) return 1;// first entry in SA should be sentinel
...
//==============================================================================
std::vector<int8_t> make_input(int32_t numUniqueSymbols, int32_t size) {
    std::vector<int8_t> input;
    input.reserve(size);
    input.resize(size);
    for (auto &e : input) e = rand() % numUniqueSymbols;
    return input;
}
...
void print_usage() {
    ...
    std::cout << "usage: msufsort [s|l] input [num threads]" << std::endl;
...
//==============================================================================
int32_t main(int32_t argumentCount, char const **inputArguments) {
    try {
        if (argumentCount < 3) {
            print_usage();
            return 0;
        ...
        enum task_type { suffix_array, lcp_array, test_mode, invalid };

        task_type taskType = invalid;

        std::string task(inputArguments[1]);
        if ((task == "s") || (task == "S")) taskType = suffix_array;
        if ((task == "l") || (task == "L")) taskType = lcp_array;
        if ((task == "t") || (task == "T")) taskType = test_mode;
        if (taskType == invalid) {
            print_usage();
            return 0;
        }
        ...
        case test_mode: {
            ...
            for (auto numUniqueSymbols = 1; ((!errorCount) && (numUniqueSymbols < 0x100)); ++numUniqueSymbols) {
                for (auto inputSize = 1; ((!errorCount) && (inputSize < (1 << 10))); ++inputSize) {
                    for (int32_t numWorkerThreads = 1; numWorkerThreads < (int32_t)std::thread::hardware_concurrency();
                         ++numWorkerThreads) {
                        srand(numUniqueSymbols * inputSize * numWorkerThreads);
                        std::cout << "sa test: num unique symbols = " << numUniqueSymbols
                                  << ", input size = " << inputSize << ", num threads = " << numWorkerThreads
                                  << std::endl;

                        auto input = make_input(numUniqueSymbols, inputSize);
                        auto suffixArray =
                            ::maniscalco::make_suffix_array(input.begin(), input.end(), numWorkerThreads);
                        // validate
                        errorCount = validate_suffix_array(input, suffixArray);
                        if (errorCount) std::cout << "**** ERRORS DETECTED (" << errorCount << ") **** " << std::endl;
                    }
                }
            }
            ...
            for (auto numUniqueSymbols = 1; ((!errorCount) && (numUniqueSymbols < 0x100)); ++numUniqueSymbols) {
                for (auto inputSize = 1; ((!errorCount) && (inputSize < (1 << 10))); ++inputSize) {
                    for (int32_t numWorkerThreads = 1;
                         ((!errorCount) && (numWorkerThreads <= (int32_t)std::thread::hardware_concurrency()));
                         ++numWorkerThreads) {
                        srand(numUniqueSymbols * inputSize * numWorkerThreads);
                        std::cout << "bwt test: num unique symbols = " << numUniqueSymbols
                                  << ", input size = " << inputSize << ", num threads = " << numWorkerThreads
                                  << std::endl;

                        auto input = make_input(numUniqueSymbols, inputSize);
                        auto copyOfInput = input;
                        auto sentinelIndex = ::maniscalco::forward_burrows_wheeler_transform(
                            input.begin(), input.end(), numWorkerThreads);
                        // validate
                        ::maniscalco::reverse_burrows_wheeler_transform(
                            input.begin(), input.end(), sentinelIndex, numWorkerThreads);
